...
import { Project, Workspace, KanbanColumn } from '../types';
    ...
    <div className="w-[260px] flex-shrink-0 flex flex-col bg-[#F9FAFB] p-4 border-r border-slate-200 h-screen">
      ...
      <div className="flex-1 overflow-y-auto space-y-8 pr-1 custom-scrollbar">
        <nav className="space-y-1">
          ...
          <div className="pt-6 pb-2 px-3 text-[10px] font-bold text-slate-400 uppercase tracking-[0.1em]">Core Views</div>
        </nav>
      ...
      <div className="mt-auto pt-6 space-y-4">
        <nav className="space-y-1">
          ...
          <button
            onClick={onSignOut}
            className="w-full flex items-center px-3 py-2 text-[13px] font-semibold rounded-lg text-slate-500 hover:bg-red-50 hover:text-red-600 border border-transparent transition-colors group"
          >
            <span className="mr-3 flex-shrink-0 flex items-center justify-center w-4 h-4 text-slate-400 group-hover:text-red-600">
      ...
      {showWorkspaceDeleteConfirm && (
        <div className="fixed inset-0 z-[110] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-[4px]" onClick={() => setShowWorkspaceDeleteConfirm(false)}>
          <div className="bg-white rounded-[2rem] shadow-2xl border border-slate-200 max-w-sm w-full p-8 animate-in zoom-in-95 duration-200" onClick={(e) => e.stopPropagation()}>
            ...
            <p className="text-sm text-slate-500 mb-8 leading-relaxed font-medium">Are you sure? This will permanently delete "{editingWorkspace?.name}" and all its content.</p>
            <div className="flex space-x-3">
              <button onClick={() => setShowWorkspaceDeleteConfirm(false)} className="flex-1 px-4 py-3 text-sm font-bold text-slate-700 bg-white border border-slate-200 rounded-xl hover:bg-slate-50 transition-colors shadow-sm">Cancel</button>
              <button onClick={confirmDeleteWorkspace} className="flex-1 px-4 py-3 text-sm font-bold text-white bg-red-600 rounded-xl hover:bg-red-700 transition-colors shadow-md">Delete</button>
            </div>
          </div>
        </div>
      )}
            ...
            <form onSubmit={handleSaveProject} className="space-y-6">
              <div className="space-y-1.5">
                ...
                <input 
                  ...
                  className="w-full px-4 py-2.5 border border-slate-200 bg-white text-slate-900 rounded-xl focus:ring-2 focus:ring-blue-500 outline-none text-sm placeholder-slate-400 shadow-sm font-medium"
                />
              </div>
              <div className="space-y-1.5">
      ...
      {showDeleteConfirm && (
        <div className="fixed inset-0 z-[110] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-[4px]" onClick={() => setShowDeleteConfirm(false)}>
          <div className="bg-white rounded-[2rem] shadow-2xl border border-slate-200 max-w-sm w-full p-8 animate-in zoom-in-95 duration-200" onClick={(e) => e.stopPropagation()}>
            ...
            <p className="text-sm text-slate-500 mb-8 leading-relaxed font-medium">Are you sure? This will remove the project category from its tasks.</p>
            <div className="flex space-x-3">
